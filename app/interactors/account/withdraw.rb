module Account
  class Withdraw
    ...
    def validate_withdraw(extract_withdraw)
      context.fail!(status: 422, message: 'impossible to withdraw from closed account.') if context.account.status.eql?(Bank::Model::Account.statuses[:closed])
    ...
    def create_extract_with_draw
      extract_deposit = Bank::Model::Extract.factory_extract({ value: context.withdraw[:withdraw], operation_type: Bank::Model::Extract.operation_types[:withdraw]} )
